...
**値に基づく動作:**
- **`false`（デフォルト）**: 解析エラーが発生した場合、ジョブは一時停止しません。代わりに、コンシューマーオフセットが更新されて問題のあるバッチがスキップされ、後続のレコードの処理が続行されます。これにより、不正な形式のデータに遭遇した際にジョブが無限リトライループに陥ることを防ぎます。問題のあるレコードを含むバッチ全体がスキップされることに注意してください（個々のレコードだけではありません）。
- **`true`**: 解析エラーが発生した場合、ジョブは一時停止し、コンシューマーオフセットは更新されません。これにより、ジョブを再開する前に問題を調査することができます。

#### `errors.tolerance`

**必須**: いいえ\
**説明**: Routine Load ジョブのエラー許容モードを指定します（Kafka Connect のエラー処理に類似）。有効な値: `none` と `all`。デフォルト値: `none`。このパラメーターは v3.5.0 以降でサポートされています。

**値に基づく動作:**
- **`none`（デフォルト）**: エラー許容は無効です。ジョブは `pause_on_fatal_parse_error` 設定に従って動作します。
- **`all`**: エラー許容が有効です。解析エラーが発生した場合、問題のあるバッチはスキップされ、コンシューマーオフセットが更新されます。これにより自動的に `pause_on_fatal_parse_error` が `false` に設定されます。

:::note
このパラメーターは、より直感的で Kafka Connect 互換のエラー処理設定方法を提供します。`errors.tolerance = "all"` を設定することは、`pause_on_fatal_parse_error = "false"` を設定することと同等です。
:::

#### `errors.deadletterqueue.topic.name`

**必須**: いいえ\
**説明**: デッドレターキュー（DLQ）の Kafka トピック名。`errors.tolerance = "all"` と一緒に指定すると、問題のあるレコードが後で分析するためにこのトピックに送信されます。このパラメーターは v3.5.0 以降でサポートされています。

:::caution
**現在の制限**: v3.5.0 時点では、このパラメーターは解析および保存されますが、実際の DLQ 機能（DLQ トピックへのメッセージ送信）はまだ実装されていません。これは将来のリリースで計画されています。現在、問題のあるバッチは単にスキップされます。
:::

#### `errors.log.enable`

**必須**: いいえ\
**説明**: エラーレコードをログに記録するかどうかを指定します。有効な値: `true` と `false`。デフォルト値: `true`。このパラメーターは v3.5.0 以降でサポートされています。

有効にすると、解析エラーが発生し、ジョブがエラーを許容するように設定されている場合（`errors.tolerance = "all"` または `pause_on_fatal_parse_error = "false"` 経由）、エラー情報がログに記録されます。
